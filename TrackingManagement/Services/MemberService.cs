...
namespace TrackingManagement.Services
{
    public class MemberService : IMemberService
    {
        ...
        public int createEscort(Escort escort)
        {
            _db.Escorts.Add(escort);
            var rfid = _db.Rfids.Where(item => item.Id == escort.Rfidid).FirstOrDefault();
            if (rfid != null)
            {
                rfid.IsDistributed = true;
            }
        ...
        public async Task<PagedResponse<List<Escort>>> getEscorts(MemberFilter memberFilter, PaginationFilter paginationFilter, List<int> unitIds)
        {
            var page = paginationFilter.Page;
            var record = paginationFilter.Record;
            var total = await _db.Escorts
                                    .Where(item => unitIds.Contains((int)item.UnitId))
                                    .AutoFilter(memberFilter).CountAsync();
            var query = _db.Escorts
                                    .Where(item => unitIds.Contains((int)item.UnitId))
                                    .AutoFilter(memberFilter)
                                    .Include(item => item.Rfid)
                                    .Include(item => item.Unit);
            ...
            {
                var queryRs = await query.Skip((page - 1) * record).Take(record).ToListAsync();
                return new PagedResponse<List<Escort>>(queryRs, total);
            }
            else
            {
                var queryRs = await query.ToListAsync();
        ...
        public int updateEscort(int escortId, UpdateMember updateMember)
        {
            var escort = _db.Escorts.Where(item => item.Id == escortId).FirstOrDefault();
            if (updateMember.Rfidid != null)
            {
                var newRfid = _db.Rfids.Where(item => item.Id == updateMember.Rfidid).FirstOrDefault();
                if (escort.Rfidid != null)
                {
                    if (escort.Rfidid != updateMember.Rfidid)
                    {

                        var currentRfid = _db.Rfids.Where(item => item.Id == escort.Rfidid).FirstOrDefault();
                        currentRfid.IsDistributed = false;
                    }
                }
                newRfid.IsDistributed = true;
            }
            var udpateRs = EntityUtils.updateRecord(escort, updateMember);
            return _db.SaveChanges();
        }
    }
}

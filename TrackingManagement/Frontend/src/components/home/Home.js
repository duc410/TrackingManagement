...
import DetailInfoCarBox from "./DetailInfoCarBox";
...
let carMarkerICon = (carsStatus) => {
  if (carsStatus.carStatus === 0) return 10;
  if (carsStatus.carStatus === 1) return 9;
  if (carsStatus.carStatus === 2) return 8;
};
...
const Home = () => {
  ...
  const getCarsStatus = (firstTime = true, map) => {
    makeRequest({
      ...
      setDisplayCarsStt(
        res.data.sort((a, b) => {
          return b.isSos - a.isSos || a.carStatus - b.carStatus;
        })
      );
      console.log(carsStatus)
      console.log(displayCarsStt)
      if (firstTime) {
        let boundBuilder = new google.maps.LatLngBounds();
        res.data.forEach((car) => {
          boundBuilder.extend(
            new google.maps.LatLng({ lat: parseFloat(car.gpsLat), lng: parseFloat(car.gpsLon) })
          );
        });
  ...
  return (
    <Row style={{ height: "100%" }} gutter={[8, 0]}>
                          ...
                          {displayCarsStt.map((item) => (
                            ...
                            >
                              <div
                                ...
                                onClick={() => {
                                  onSelectCar(item);
                                  handleActiveMarker(item.car.id);
                                }}
                              >
          ...
          >
            {carsStatus.map((car) => (
              <>
                <Marker
                  key={car.id}
                  position={{ lat: parseFloat(car.gpsLat), lng: parseFloat(car.gpsLon) }}
            ...
            {activeInfoWindow ? (
              <InfoWindow
                position={{
                  lat: parseFloat(activeInfoWindow.gpsLat),
                  lng: parseFloat(activeInfoWindow.gpsLon),
                }}
            ...
            {selectedCar ? (
              carsStatus
                ...
                .map((item) => (
                  <InfoBox
                    position={{ lat: parseFloat(item.gpsLat), lng: parseFloat(item.gpsLon) }}
                    options={{
            ...
            {selectedHistory ? (
              <Marker
                ...
                position={{
                  lat: parseFloat(selectedHistory.gpsLat),
                  lng: parseFloat(selectedHistory.gpsLon),
                }}
                icon={{
      ...
      <Modal
        ...
        style={{ top: 20 }}
      >
        <div style={{ minHeight: 800, minWidth: 1200 }}>
          <DetailInfoCarBox
            carInfo={activeInfoWindow}
            units={assets.userUnits}
          />

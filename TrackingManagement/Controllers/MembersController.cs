...
namespace TrackingManagement.Controllers
{
    ...
    public class MembersController : ControllerBase
    {
        ...
        [HttpGet]
        [Route("escorts")]
        public async Task<IActionResult> getEscort([FromQuery] MemberFilter memberFilter, [FromQuery] PaginationFilter paginationFilter)
        {
            List<int> unitIds = (List<int>)HttpContext.Items["unitIds"];
            var rs = await _memberService.getEscorts(memberFilter, paginationFilter, unitIds);
            return Ok(rs);
        }
        ...
        [HttpPost]
        [Route("escorts")]
        public IActionResult createEscort([FromBody] Escort escort)
        {
            var createdResult = _memberService.createEscort(escort);
            var rs = new BaseResponse<string>();
            if (createdResult == 1)
            {
                rs.Message = "Tạo mới thành công";
        ...
        [HttpPut]
        [Route("escorts/{id}")]
        public IActionResult updateEscort(int id, [FromBody] UpdateMember updateMember)
        {
            var updateRs = _memberService.updateEscort(id, updateMember);
            var response = new BaseResponse<String>();
            if (updateRs == 1)
            {
                response.Message = "Cập nhật thành công";

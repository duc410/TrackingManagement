        ...
        public async Task Invoke(HttpContext httpContext)
        {
            ...
            if (route != null)
            {
                ...
                if (allowCurrentPath == null)
                {
                    ...
                    //var userPermissions = _db.ScopePermissions.Where(item => item.ScopeId == scopeId)
                    //                                          .Include(item => item.Permission)
                    //                                          .Select(item => new
                    //                                          {
                    //                                              Method= item.Permission.Method,
                    //                                              PermissionUrl = item.Permission.Url,
                    //                                              Allowed = item.Allowed
                    //                                          })
                    //                                          .ToList();

                    //var routePermissons = new
                    //{
                    ...
                    //bool test = !userPermissions.Contains(routePermissons);
                    //if (!userPermissions.Contains(routePermissons))
                    //{

                    //    string html = "<h1>CAM KHONG DUOC TRUY CAP</h1>";
                    //    var rsJson = JsonConvert.SerializeObject(html);
                    //    httpContext.Response.StatusCode = StatusCodes.Status403Forbidden;
                    //    httpContext.Response.WriteAsync(rsJson); ;
                    //}
